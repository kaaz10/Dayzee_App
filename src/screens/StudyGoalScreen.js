  ...
  const StudyGoalScreen = () => {
    ...
    const loadSavedData = async () => {
      try {
        const savedCompletedDays = await AsyncStorage.getItem('studyCompletedDays');
        const savedCompletedToday = await AsyncStorage.getItem('studyCompletedToday');
        const savedWeekStart = await AsyncStorage.getItem('studyWeekStart');
        ...
        console.error('Error loading saved study data:', error);
      }
    };
    ...
    const checkAndResetWeek = async () => {
      try {
        ...
        if (savedWeekStart !== currentWeekStart) {
          ...
          await AsyncStorage.setItem('studyCompletedDays', JSON.stringify(newCompletedDays));
          await AsyncStorage.setItem('studyCompletedToday', JSON.stringify(false));
          await AsyncStorage.setItem('studyWeekStart', currentWeekStart);
        }
      } catch (error) {
        console.error('Error checking study week:', error);
      }
    };
    ...
    const fetchNearbyPlaces = async (coords) => {
      ...
      const radius = 2000;
      const types = 'cafe|library|school|college|university';
    ...
    const toggleCompletionStatus = async () => {
      try {
        ...
        await AsyncStorage.setItem('studyCompletedToday', JSON.stringify(newCompletedToday));
        await AsyncStorage.setItem('studyCompletedDays', JSON.stringify(newCompletedDays));
      } catch (error) {
        console.error('Error saving study completion status:', error);
      }
    };
